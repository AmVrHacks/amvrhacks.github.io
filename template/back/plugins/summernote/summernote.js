...
(function (factory) {
  ...
  var settings = {
    ...
    options: {
      ...
      toolbar: [
        ...
        ['snippet', ['leftsnippet', 'rightsnippet']],
       ...
       * @return {String}
       */
      onCreateSnippet: function (sSnippetUrl) {
  ...
  var Editor = function () {
    ...
    this.insertVideo = function ($editable, sUrl) {
      ...
      if (ytMatch && ytMatch[2].length === 11) {
        ...
        $video = $('<iframe>')
          ...
          .attr('width', '100%').attr('height', '710')
          .attr('scrolling', 'no')
     ...
     * @param {Object} options
     */
    this.createSnippet = function ($editable, SnippetInfo, options) {
      var SnippetUrl = SnippetInfo.url;
      var SnippetText = SnippetInfo.text;
      var isNewWindow = SnippetInfo.newWindow;
      var rng = SnippetInfo.range;

      recordUndo($editable);

      if (options.onCreateLink) {
        SnippetUrl = options.onCreateSnippet(SnippetUrl);
      }

      rng = rng.deleteContents();

      // Create a new link when there is no anchor on range.
      var anchor = rng.insertNode($(SnippetText)[0], true);
     ...
     * @return {Object}
     */
    this.getSnippetInfo = function ($editable) {
      $editable.focus();
      ...
      return {
        range: rng,
        text: '<div class="col-md-6" style="color:red;font-size:20px;"><i>Snippet Set</i></div>',
        isNewWindow: $anchor.length ? $anchor.attr('target') === '_blank' : true,
        url: $anchor.length ? $anchor.attr('href') : ''
      };
    };
  ...
  var Dialog = function () {
     ...
     * @return {Promise}
     */
    this.showSnippetDialog = function ($editable, $dialog, SnippetInfo) {
      return $.Deferred(function (deferred) {
        var $SnippetDialog = $dialog.find('.note-snippet-dialog');

        var $SnippetText = $SnippetDialog.find('.note-snippet-text'),
        $SnippetUrl = $SnippetDialog.find('.note-snippet-url'),
        $SnippetBtn = $SnippetDialog.find('.note-snippet-btn'),
        $openInNewWindow = $SnippetDialog.find('input[type=checkbox]');

        $SnippetDialog.one('shown.bs.modal', function () {
          $SnippetText.val(SnippetInfo.text);

          $SnippetText.keyup(function () {
            // if linktext was modified by keyup,
            // stop cloning text from linkUrl
            SnippetInfo.text = $SnippetText.val();
          });

          // if no url was given, copy text to url
          if (!SnippetInfo.url) {
            SnippetInfo.url = SnippetInfo.text;
            toggleBtn($SnippetBtn, SnippetInfo.text);
          }

          $SnippetUrl.keyup(function () {
            toggleBtn($SnippetBtn, $SnippetUrl.val());
            // display same link on `Text to display` input
            // when create a new link
            if (!SnippetInfo.text) {
              $SnippetText.val($SnippetUrl.val());
            }
          }).val(SnippetInfo.url).trigger('focus').trigger('select');

          $openInNewWindow.prop('checked', SnippetInfo.newWindow);

          $SnippetBtn.one('click', function (event) {
            event.preventDefault();

            deferred.resolve({
              range: SnippetInfo.range,
              url: $SnippetUrl.val(),
              text: $SnippetText.val(),
              newWindow: $openInNewWindow.is(':checked')
            });
            $SnippetDialog.modal('hide');
          });
        }).one('hidden.bs.modal', function () {
          // dettach events
          $SnippetText.off('keyup');
          $SnippetUrl.off('keyup');
          $SnippetBtn.off('click');
  ...
  var EventHandler = function () {
    ...
    var commands = {
       ...
       */
      showSnippetDialog: function (layoutInfo) {
        var $editor = layoutInfo.editor(),
            $dialog = layoutInfo.dialog(),
            $editable = layoutInfo.editable(),
            SnippetInfo = editor.getSnippetInfo($editable);

        var options = $editor.data('options');
        editor.saveRange($editable);
        dialog.showSnippetDialog($editable, $dialog, SnippetInfo).then(function (SnippetInfo) {
          editor.restoreRange($editable);
          editor.createSnippet($editable, SnippetInfo, options);
  ...
  var Renderer = function () {
    ...
    var tplButtonInfo = {
      ...
      rightsnippet: function (lang) {
        return tplIconButton('fa fa-caret-square-o-right icon-plus', {
          event: 'showSnippetDialog',
          title: 'Right Snippet',
          hide: true
        });
      },
      leftsnippet: function (lang) {
        return tplIconButton('fa fa-caret-square-o-left icon-plus', {
          event: 'showSnippetDialog',
          title: 'Left Snippet',
          hide: true
        });
    ...
    var tplDialogs = function (lang, options) {
      ...
      var tplSnippetDialog = function () {
        var body = '<div class="form-group">' +
                     '<label>' + lang.snippet.textToDisplay + '</label>' +
                     '<input class="note-snippet-text form-control span12" type="text" />' +
                   '</div>' +
                   '<div class="form-group" style="display:none;">' +
                     '<label>' + lang.snippet.url + '</label>' +
                     '<input class="note-snippet-url form-control span12" type="text" />' +
                   '</div>';
        var footer = '<button href="#" class="btn btn-primary note-snippet-btn disabled" disabled>' + lang.snippet.insert + '</button>';
        return tplDialog('note-snippet-dialog', lang.snippet.insert, body, footer);
      };
